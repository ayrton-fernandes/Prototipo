...
export default function TargetOperationDocumentationPage() {
  ...
  const {
    ...
    templateName: "Documentação da Operação",
    sectionLabel: "Documentação da Operação",
  });

  if (!loading && !hasAccessToTab("DOCUMENTACAO_DA_OPERACAO")) {
    return (
      <>
        <TargetSectionsHeader activeTabId="DOCUMENTACAO_DA_OPERACAO" />

        <Card className="prontuario-surface-card">
          <div className="flex flex-col gap-4">
            <Typography variant="h3">Documentação da Operação</Typography>
  ...
  if (loading || recordLoading) {
    return (
      <>
        ...
        <Card className="prontuario-surface-card">
          <Typography variant="p">Carregando documentação da operação...</Typography>
        </Card>
      </>
    );
  }
  ...
  if (errorMessage || !target) {
    return (
      <>
        ...
        <Card className="prontuario-surface-card">
          <div className="flex flex-col gap-4">
            <Typography variant="h3">Documentação da Operação do Alvo</Typography>
      ...
      <div className="prontuario-target-page cpo-text-on-light grid gap-8 xl:grid-cols-[minmax(0,1fr)_18rem] xl:items-start">
        <main className="flex min-w-0 flex-col gap-8">
          <section className="flex flex-col gap-2">
            <Typography variant="h1">Documentação da Operação do Alvo</Typography>
            <Typography variant="p" className="max-w-3xl">
              Visualize e {canEditContent ? "edite" : "consulte"} a documentação associada à operação deste alvo, com campos carregados dinamicamente do template do backend.
            </Typography>
          </section>
