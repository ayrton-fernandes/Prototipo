...
export default function TargetDocumentsPage() {
  ...
  if (!loading && !hasAccessToTab("DOCUMENTOS_GENERICOS")) {
    return (
      <>
        <TargetSectionsHeader activeTabId="DOCUMENTOS_GENERICOS" />

        <Card className="prontuario-surface-card">
          <div className="flex flex-col gap-4">
            <Typography variant="h3">Documentos Genéricos</Typography>
  ...
  if (loading || recordLoading) {
    return (
      <>
        ...
        <Card className="prontuario-surface-card">
          <Typography variant="p">Carregando documentos do alvo...</Typography>
        </Card>
      </>
    );
  }
  ...
  if (errorMessage || !target) {
    return (
      <>
        ...
        <Card className="prontuario-surface-card">
          <div className="flex flex-col gap-4">
            <Typography variant="h3">Documentos Genéricos do Alvo</Typography>
      ...
      <div className="prontuario-target-page cpo-text-on-light grid gap-8 xl:grid-cols-[minmax(0,1fr)_18rem] xl:items-start">
        <main className="flex min-w-0 flex-col gap-8">
          <section className="flex flex-col gap-2">
            <Typography variant="h1">Documentos Genéricos do Alvo</Typography>
            <Typography variant="p" className="max-w-3xl">
              Visualize e {canEditContent ? "edite" : "consulte"} os documentos genéricos associados ao alvo, com campos carregados dinamicamente do template do backend.
            </Typography>
          </section>
